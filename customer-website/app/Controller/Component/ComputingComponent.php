	...
	*/
	public function check_session(){
		...
		if($this->Session->check('data')){
			...
			if($current_time > $exp_time){ // het han time out
			...
			$info = $Account->find('first',array('conditions' => array('Account.id' => $this->Auth->user('id'))));
				// pr($info);die;
			if(empty($info['Account']['lname']) || empty($info['Account']['fname'])||empty($info['Account']['email']) || empty($info['Account']['phonenumber'])||empty($info['Account']['add_contact']) ||empty($info['Account']['country']) || empty($info['Account']['nickname'])|| empty($info['Account']['address'])){
				if(!empty($this->Auth->user('proxy'))){
					return $this->Controller->redirect($fontend_url.'/members/set_info?account=group&id='.$info['Account']['id']);
				}else{
					return $this->Controller->redirect($fontend_url.'/members/set_info?account=person&id='.$info['Account']['id']);
				}
			}
			else{
				$this->login();
